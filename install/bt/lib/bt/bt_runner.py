...
class ScanEnvironment(py_trees.behaviour.Behaviour):
    """
    Rotates the robot a full 360 degrees at startup and records the best
    bearing for each colour seen (largest area wins).

    Scan state (_rotated, _best_areas) is stored on the node so that if
    the behaviour is interrupted (e.g. by a battery event) and re-initialised,
    it resumes rather than restarting from zero.
    """

    FULL_ROTATION = 2.0 * math.pi
    SCAN_SPEED    = 0.3   # rad/s
    HALF_FOV      = 0.52  # radians (~30 deg half-FOV)
    ...
    def initialise(self):
        # Only reset progress if this is a genuine first start.
        # If _scan_rotated already exists on the node, we were interrupted
    ...
    def update(self):
        ...
        if self.node._scan_rotated >= self.FULL_ROTATION:
            self.node.cmd_vel_pub.publish(Twist())
            # Clean up temporary scan state from node
    ...
    def terminate(self, new_status):
        # Stop the robot but do NOT delete _scan_rotated — preserve progress
        self.node.cmd_vel_pub.publish(Twist())
...
class NavigateToColour(py_trees.behaviour.Behaviour):
    ...
    Phase 1 - TURN_TO_BEARING:
      Rotate to the bearing recorded during the startup scan.

    Phase 2 - SEARCH:
      Rotate slowly until the target colour is visible.

    Phase 3 - ALIGN:
      Colour visible but off-centre — rotate to centre it.

    Phase 4 - DRIVE:
      Colour centred — drive forward until LiDAR <= stop_distance.
    """
    ...
    def initialise(self):
        ...
        if self.target_colour in self.node.colour_bearings:
            self._phase = 'TURN_TO_BEARING'
            self.node.get_logger().info(
                f'NavigateToColour [{self.target_colour}]: turning to bearing '
                f'{math.degrees(self.node.colour_bearings[self.target_colour]):.1f} deg'
            )
        else:
            self._phase = 'SEARCH'
            self.node.get_logger().info(
                f'NavigateToColour [{self.target_colour}]: no bearing — blind search'
            )
    ...
    def update(self):
        ...
        if self._phase == 'TURN_TO_BEARING':
            ...
            if bearing is None:
                ...
                if abs(heading_error) < self.BEARING_THRESHOLD:
                    self._phase = 'ALIGN' if on_target else 'SEARCH'
                    self.node.get_logger().info(
        ...
        if self._phase == 'ALIGN':
            if not on_target:
                self._phase = 'SEARCH'
            elif abs(self.node.colour_offset_x) <= self.ALIGN_THRESHOLD:
                self._phase = 'DRIVE'
            else:
                twist.angular.z = -0.5 * self.node.colour_offset_x
                self.node.get_logger().info(
                    f'Aligning to {self.target_colour}: '
                    f'offset={self.node.colour_offset_x:.2f}',
                    throttle_duration_sec=1.0
                )
        ...
        if self._phase == 'DRIVE':
            if not on_target:
                ...
                twist.angular.z = -0.3 * self.node.colour_offset_x
                self.node.get_logger().info(
...
class BTRunner(Node):
    ...
    def _build_tree(self):
        ...
        # Startup scan — persists progress across interruptions
        startup_scan = py_trees.composites.Selector(
            name='StartupScanWrapper',
            memory=False,
            children=[
